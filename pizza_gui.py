...
class gui():
    ...
    def __init__(self, args):
        ...
        # 10 pizzas, 27 toppings, each has [x, y, topping type] (x and y wrt pizza center)
        self.pizzas = np.zeros((10, 24, 3))
        self.pizza_id = 0
        # 10 pizzas, x, y coordinates of cut intscn wrt pizza center and angle of cut in the 0-45 degree range
        self.cuts = list(np.zeros((10, 3)))
        self.type_p = ""
        self.pizzas_drawn = 0
        self.options_player = [
            "custom_player", "default_player", "p1", "p2", "p3", "p4", "p5", "p6"]
        self.options_topping = [2, 3, 4]
        self.topping_id = 0  # Current topping id
        # if this goes badly set it to 20     #Just to arrange the difference in sizes of both display screens of 1 and 10 pizzas
        self.template_adjustor = 2*self.multiplier
...
# creating Canvas
    def create_canvas_for_shapes(self):
        self.canvas = Tkinter.Canvas(
            self.root, width=35*self.multiplier, height=27*self.multiplier, bg='#E5E6DC')
    ...
    def clickevent_topping(self, event):
        ...
        clash_exists = pizza_calculations.clash_exists(
            x_relative, y_relative, pizza, self.topping_id)
        if clash_exists:
            self.label.config(text="Invalid placement. Try again.")
        else:
            self.canvas.create_oval(x_coord - (0.375*self.multiplier), y_coord - (0.375*self.multiplier),
                                    x_coord + (0.375*self.multiplier), y_coord + (0.375*self.multiplier), fill=color_top)
            self.pizzas[self.pizza_id][self.topping_id][0] = (
                x_coord - self.x)/self.multiplier
            self.pizzas[self.pizza_id][self.topping_id][1] = - \
                (y_coord - self.y)/self.multiplier
            self.pizzas[self.pizza_id][self.topping_id][2] = topping_number
            # Used to track overall which toppings should be shown on a single pizza
            self.topping_id = self.topping_id + 1
            self.label.config(text="Make pizza number "+str(self.pizza_id) + " with type "+str(
                topping_number) + " of topping and " + str(24-self.topping_id) + " toppings left.")
            if self.topping_id >= 24:
                ...
                if self.pizza_id < constants.number_of_initial_pizzas:
                    ...
                    self.button.destroy()
                    self.button = Button(
                        self.root, text="Start Serving!!", command=self.pizza_choice)
    ...
    def clickevent_pizzacenter(self, event):
        ...
        if (x_coord-self.x)**2 + (y_coord-self.y)**2 > (6*self.multiplier)**2:
            ...
            self.canvas.create_oval(x_coord - (0.1*self.multiplier), y_coord - (0.1*self.multiplier),
                                    x_coord + (0.1*self.multiplier), y_coord + (0.1*self.multiplier), fill="black")
            self.canvas.unbind("<Button-1>")
    ...
    def clickevent_cut(self, event):
        ...
        if x_coord == piz_center[0]:
            ...
            thetaabs = np.arctan(-(y_coord -
                                 piz_center[1])/(x_coord - piz_center[0]))
            if (x_coord - piz_center[0]) <= 0:
                thetaabs = thetaabs + np.pi
        if self.x == piz_center[0]:
            centangle = 0
        else:
            centangle = np.arctan(-(piz_center[1] -
                                  self.y) / (piz_center[0]-self.x))
        ...
        results, temp = self.calculator.ratio_calculator(
        ...
        self.label.config(text="Preferences are : " + str(np.round(
            self.preferences[self.customer_id])) + ". Results are " + str(np.round(results, 2)) + ".")
        ...
        if game_over_indic == 0:
            self.button = Button(
                self.root, text="Next Customer", command=self.pizza_choice)
            self.button.place(x=153, y=20)
        else:
            self.button = Button(
                self.root, text="See results", command=self.see_score)
            self.button.place(x=153, y=20)
        self.customer_id = self.customer_id + 1

    def draw_pizza(self, pizza_id):
        self.canvas.create_oval(self.x - (self.multiplier*6), self.y - (self.multiplier*6),
                                self.x + (self.multiplier*6), self.y + (self.multiplier*6), fill="navajo white")
        self.canvas.create_oval(self.x - (self.multiplier*5.5), self.y - (self.multiplier*5.5),
                                self.x + (self.multiplier*5.5), self.y + (self.multiplier*5.5), fill="red3")
        for pizza_top_i in list(self.pizzas[pizza_id]):
            if pizza_top_i[2] != 0:  # If they have been initialized
                if pizza_top_i[2] == 1:
                    ...
                    color_top = "yellow"
                self.canvas.create_oval(self.x + pizza_top_i[0]*self.multiplier - (0.375*self.multiplier), self.y - pizza_top_i[1]*self.multiplier - (
                    0.375*self.multiplier), self.x + pizza_top_i[0]*self.multiplier + (0.375*self.multiplier), self.y - pizza_top_i[1]*self.multiplier + (0.375*self.multiplier), fill=color_top)
    ...
    def draw_pizzas(self):
        ...
        for pizza_id in range(constants.number_of_initial_pizzas):
            ...
            self.canvas.create_oval(self.template_adjustor + x_small - (multiplier_small*6), -self.template_adjustor + y_small - (multiplier_small*6),
                                    self.template_adjustor + x_small + (multiplier_small*6), -self.template_adjustor + y_small + (multiplier_small*6), fill="navajo white")
            self.canvas.create_oval(self.template_adjustor + x_small - (multiplier_small*5.5), -self.template_adjustor + y_small - (multiplier_small*5.5),
                                    self.template_adjustor + x_small + (multiplier_small*5.5), -self.template_adjustor + y_small + (multiplier_small*5.5), fill="red3")
            for pizza_top_i in list(self.pizzas[pizza_id]):
                if pizza_top_i[2] != 0:
                    if pizza_top_i[2] == 1:
                        color_top = "black"
                        ...
                        color_top = "yellow"
                    self.canvas.create_oval(self.template_adjustor + x_small + pizza_top_i[0]*multiplier_small - (0.375*multiplier_small), -self.template_adjustor + y_small - pizza_top_i[1]*multiplier_small - (
                        0.375*multiplier_small), self.template_adjustor + x_small + pizza_top_i[0]*multiplier_small + (0.375*multiplier_small), -self.template_adjustor + y_small - pizza_top_i[1]*multiplier_small + (0.375*multiplier_small), fill=color_top)
            self.canvas.create_text(self.template_adjustor + x_small, -self.template_adjustor + y_small +
    ...
    def auto_pizza(self):
        ...
        self.label.config(text="Make pizza number "+str(self.pizza_id) + " with "+str(
            self.num_toppings) + " types of toppings and " + str(24-self.topping_id) + " toppings left.")
        if self.pizza_id < constants.number_of_initial_pizzas:
            self.draw_pizza(self.pizza_id)
        else:
            self.canvas.unbind("<Button-1>")
    ...
    def draw_cuts(self, center, theta, color="black"):  # theta is in radians here
        dist_centers = math.sqrt(
            (center[0]-self.x)**2 + (center[1] - self.y)**2)
        if dist_centers/self.multiplier >= 6:
            self.label.config(
                text="You tried to place the center outside the pizza. That's not cool bro.")
        if center[0] == self.x:
            angle_centerline = 0
        else:
            angle_centerline = math.atan(
                - (center[1] - self.y)/(center[0] - self.x))
        theta_diag = angle_centerline - theta
        sinin_1 = math.asin(math.sin(theta_diag) *
                            dist_centers/(self.multiplier*6))
        phi_1 = theta_diag - sinin_1
        phi_2 = theta_diag - math.pi + sinin_1
        if np.round(math.sin(theta_diag), 4) == 0:
            point_1 = [self.x + 6*self.multiplier*math.cos(
                angle_centerline),  self.y - 6*self.multiplier*math.sin(angle_centerline)]
            point_2 = [self.x - 6*self.multiplier*math.cos(
                angle_centerline),  self.y + 6*self.multiplier*math.sin(angle_centerline)]
        else:
        ...
        self.canvas.create_line(
            point_1[0], point_1[1], point_2[0], point_2[1], fill=color)
        ...
        if self.type_player != "custom_player":
            ...
            for i in range(constants.number_of_initial_pizzas):
                for j in range(24):
                    pizza = self.pizzas[i]
                    clash_exists = pizza_calculations.clash_exists(
                        self.pizzas[i][j][0], self.pizzas[i][j][1], pizza, j)
                    if clash_exists == True:
                        print("CLASH!", i, j,
                              self.pizzas[i][j][0], self.pizzas[i][j][1])
                        clash_exists_overall = True
            if clash_exists_overall:
                self.label.config(
                    text="Overlapping placement. You cannot serve customers now.")
                self.button.destroy()
                self.button = Button(
                    self.root, text="Exit", command=self.end_run)
                self.button.place(x=123, y=20)
            else:
                self.button = Button(
                    self.root, text="Start Serving!!", command=self.pizza_choice)
            ...
            self.button.destroy()
            self.button = Button(
                self.root, text="Autoplay turn", command=self.auto_pizza)
            self.button.place(x=123, y=20)
            if self.pizza_id >= 10:
                self.button.destroy()
                self.label.config(
                    text="Congratulations chef all your pizzas are made!!")
                self.button = Button(
                    self.root, text="Start Serving!!", command=self.pizza_choice)
                self.button.place(x=123, y=20)
                ...
                self.draw_pizza(self.pizza_id)
                self.label.config(text="Make pizza number "+str(self.pizza_id) + " with type "+str(
                    1) + " of topping and " + str(24-self.topping_id) + " toppings left.")
    ...
    def pizza_choice(self):
        ...
        if self.customer_id == constants.number_of_initial_pizzas:
            self.button = Button(
                self.root, text="See final scores", command=self.see_score)
            self.button.place(x=153, y=20)
        else:
            self.button = Button(self.root, text="Autoplay",
                                 command=self.autoplay_cuts)
            self.button.place(x=153, y=20)
            self.button_1 = Button(
                self.root, text="Manual play", command=self.choose_cuts)
            self.button_1.place(x=253, y=20)
            self.label.config(text="Customer number "+str(self.customer_id) + " with preferences "+str(
                self.preferences[self.customer_id][0]) + " and " + str(self.preferences[self.customer_id][1]))
            ...
            self.pizza_number_choice = OptionMenu(
                self.root, self.pizza_cut_choice, *options_pizza)
    ...
    def autoplay_cuts(self):
        ...
        for i in range(8):
            if i == 0 or i == 4:
                self.draw_cuts(
                    [self.cuts[pizza_id][0], self.cuts[pizza_id][1]], theta+i*np.pi/4, color="yellow")
            else:
                self.draw_cuts(
                    [self.cuts[pizza_id][0], self.cuts[pizza_id][1]], theta+i*np.pi/4)
        self.button.destroy()
        self.button_1.destroy()
        game_over_indic = 1
        for i in range(len(self.cuts)):
        ...
        if game_over_indic == 0:
            ...
            self.button.place(x=153, y=20)
            results, temp = self.calculator.ratio_calculator(
                self.pizzas[pizza_id], self.cuts[pizza_id], self.num_toppings, self.multiplier, self.x, self.y)
            self.final_preferences.append(results)
            self.label.config(text="Pizza chosen " + str(pizza_id) + ". Customer preferences : "+str(np.round(
                self.preferences[self.customer_id], 2)) + ". Obtained preferences : "+str(np.round(results, 2)))
        else:
            self.button = Button(
                self.root, text="See results", command=self.see_score)
            self.button.place(x=153, y=20)
    ...
    def see_score(self):
        B, C, U, obtained_preferences, center_offsets, slice_amount_metrics = self.calculator.final_score(
            self.pizzas, self.pizza_choice_order, self.preferences, self.cuts, self.num_toppings, self.multiplier, self.x, self.y)
        list_scores = [('Customer Number', "Pizza Number", "U",
                        "B", "C", "S", "Center offsets", "Slice metric")]
        with open("summary_log_gui.txt", "w") as f:
            ...
            for i in range(len(self.pizzas)):
                ...
                f.write('\n')
                f.write("Cut at (" + str((self.cuts[pizza_id][0]-self.x)/self.multiplier) + ", " + str(
                    (self.cuts[pizza_id][1]-self.y)/self.multiplier) + ") and angle " + str(self.cuts[pizza_id][2]) + "radians.")
                f.write('\n')
                f.write("Center offset : " + str(center_offsets[i]))
                f.write('\n')
                for j in range(24):
                    f.write("Topping " + str(self.pizzas[pizza_id][j][2]) + " at (" + str(
                        self.pizzas[pizza_id][j][0]) + "," + str(self.pizzas[pizza_id][j][1]) + ")")
                    f.write('\n')
                f.write("U : " + str(U[i]))
                f.write('\n')
                ...
                f.write('\n')
                f.write("Obtained Preferences : " +
                        str(obtained_preferences[i]))
                f.write('\n')
                f.write("Slice by slice amount differences sum : " +
                        str(slice_amount_metrics[i]))
                f.write('\n')
                f.write('\n')
                f.write('\n')
                ...
                list_scores.append((str(i+1), str(pizza_id), str(np.round(U[i].sum(), 2)), str(np.round(B[i].sum(), 2)), str(np.round(C[i].sum(), 2)), str(
                    np.round((np.sum(B[i], axis=1) - np.sum(C[i], axis=1)).sum(), 2)),  str(np.round(center_offsets[i], 2)),  str(np.round(slice_amount_metrics[i], 2))))
            ...
            f.write('\n')
            f.write("Total Slice by slice metric : " +
                    str(np.sum(slice_amount_metrics)))
            f.write('\n')
        ...
        button_global = Button(
            self.root_1, text="Exit App", command=self.end_run)
    ...
    def cut_pizzas(self):
        ...
        with open("pizza_gui.pkl", "wb") as f:
            pkl.dump(my_dict, f)
        self.button = Button(
            self.root, text="See results!!", command=self.score)
        self.button.place(x=123, y=20)
        if self.type_player != "custom_player":
            # self.pizzas = self.player.create_pizzas(self.num_toppings)      #This line should come in other pizza_game.py. There te self gets updated and transferred here.
            time.sleep(5)
            ...
            if self.pizza_id >= 10:
                ...
                self.label.config(
                    text="Congratulations chef all your pizzas are made!!")
                self.button = Button(
                    self.root, text="Start Serving!!", command=self.cut_pizzas)
                self.button.place(x=123, y=20)
                my_dict = self.convert_to_dict()
                with open("pizza_gui.pkl", "wb") as f:
                    pkl.dump(my_dict, f)
    ...
    def run(self):
        ...
        self.canvas = Tkinter.Canvas(
            self.root, width=35*self.multiplier, height=20*self.multiplier, bg='#E5E6DC')
        self.canvas.bind("<Key>", self.clickevent_topping)
        self.canvas.pack(expand='yes', fill='both')
        self.label = Label(
            self.root, text="You can customize your pizza here!")
        ...
        self.num_top.pack()
        self.type_players = OptionMenu(
            self.root, self.type_p, *self.options_player)
        self.type_players.pack()
        # Create Label
        self.draw_pizza(0)
        self.canvas.create_text(self.x, self.y, text="Hi from FlexPizza!!",
                                fill="navajo white", font=('Helvetica 23 bold'))
        self.button = Button(self.root, text="Start Game",
                             command=self.get_pizzas)
        self.button.place(x=123, y=20)
        self.label.pack()
        self.root.mainloop()
